    ...
    var page = {
        ...
        init: function ($page, param) {
                            ...
                            event: function () {// 删除当前条信息
                                learun.layer.confirm('确定要删除该笔数据吗？', function (_index) {
                                    if (_index === '1') {
                                        ...
                                        learun.httppost(config.webapi + 'learun/adms/Platform/Candidates/delete', keyValue, (data) => {
                                            learun.layer.loading(false);
                                            if (data) {// 删除数据成功
                                                learun.nav.closeCurrent();
                                                var prepage = learun.nav.getpage('Platform/Candidates');
                                                prepage.grid.reload();
                                            }
                                        });
                                    }
            ...
            $header.find('.lr-form-header-submit').on('tap', function () {
                ...
                learun.httppost(config.webapi + 'learun/adms/Platform/Candidates/save', _postData, (data) => {
                    learun.layer.loading(false);
                    if (data) {// 表单数据保存成功
                        if (keyValue) {
            ...
            if (keyValue) {
                ...
                learun.httpget(config.webapi + 'learun/adms/Platform/Candidates/form', keyValue, (data) => {
                    if (data) {
                        for (var id in data) {
                            if (data[id].length) {
        ...
        bind: function ($page, param) {
            $page.find('#F_Gender').lrpickerex({
            code: 'usersex',
            type: 'dataItem'            });
            $page.find('#F_IsGetAccommodation').lrpickerex({
            code: 'YesOrNo',
            type: 'dataItem'            });
            $page.find('#F_EnabledMark').imagepicker();

(function () {
    ...
    var getMsgHtml = function (item) {
        ...
        var imgurl = "../pulsApp/images/on-boy.jpg";
        if (learun.storage.get('userinfo').baseinfo.gender == 0) {
            imgurl = "../pulsApp/images/on-girl.jpg";
        }
    ...
    var getMsg = function (userId) {
        learun.httpget(config.webapi + "learun/adms/im/msg/list2", { otherUserId: userId, time: endTime },
            function (data) {
                if (data) {
    ...
    var page = {
        init: function ($page, param) {
            $scroll = $page.find('#lr_chat_msgcontent').pullRefresh({
                down: {
                    ...
                    callback: function () {
                        ...
                        learun.httpget(config.webapi + "learun/adms/im/msg/list", { otherUserId: param.userId, time: beginTime }, function (data) {
                            if (data) {
                                var $list = $('#lr_chat_msgcontent .lr-chat-msgcontent-list');
                                $.each(data, function (_index, _item) {
                                    if (!msgMap[_item.F_MsgId]) {
            ...
            if (param.hasHistory) {
                ...
                // 先去获取最近的十条数据
                learun.httpget(config.webapi + "learun/adms/im/msg/lastlist", param.userId, function (data) {
            ...
            $page.find('#lr_chat_sendbtn').on('tap', function () {
                ...
                if (text) {
                    ...
                    learun.httppost(config.webapi + "learun/adms/im/send", { userId: param.userId, content: text }, function (data) {
                        if (data) {
                            endTime = data.time;
                            msgMap[data.msgId] = "1";
                        }

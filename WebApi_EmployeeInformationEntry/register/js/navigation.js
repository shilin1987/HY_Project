...
(function ($, learun, fui, window) {
    ...
    function load(path) {
        ...
        if (!learun.nav.data[path]) {// 判断下当前页面是否已经加载了
            ...
            // 加载页面css代码
            learun.http.webget(filename + '.css', {}, function (res) {
                if (res !== null) {
                    fui.includeCss(res);
                }
            }, 'text');
            ...
            // 加载页面html代码
            learun.http.webget(filename + '.html', {}, function (res) {
                if (res === null) {
                    fui.dialog({ msg: '华羿信息提示：找不到页面！' });
                }
                learun.nav.data[path].strhtml = res || '';
                learun.nav.data[path].htmlLoaded = true;
            }, 'text');
            // 加载页面js代码
            learun.http.webget(filename + '.js', {}, function (res) {

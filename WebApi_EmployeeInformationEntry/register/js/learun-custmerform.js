...
(function ($, learun, window) {
    ...
    learun.custmerform = {
        loadScheme: function (schemeIds, callback) {// formIds表单主键集合,callback回调函数
            ...
            // 加载自定义表单模板
            learun.httpget(config.webapi + "learun/adms/form/scheme", req, function (data) {
                if (data) {
    ...
    // 获取自定义表单数据
    $.fn.custmerformGet = function (flag) {// true 就不检验表单字段
        var res = {};
        ...
        currentInfo: {
            ...
            set: function (compont, value, $container) {
                if (value) {
                    ...
                    $container.find('#' + compont.id).attr('data-flag','1');
                    switch (compont.dataType) {
                        case 'company':
                            compont.value = value;
                            if (compont.isHide !== '1') {

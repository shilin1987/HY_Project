 ...
 * 创建人：华羿-前端开发组
 * 日 期：2018.7.12
 * 描 述：华羿移动端框架(ADMS) 自定义表单发布功能-表单页
 */
(function () {
    var keyValue = '';
    var formSchemeId = '';
    ...
    var page = {
        init: function ($page, param) {
                            ...
                            event: function () {// 删除当前条信息
                                learun.layer.confirm('确定要删除该笔数据吗？', function (_index) {
                                    if (_index === '1') {
                                        ...
                                        learun.httppost(config.webapi + "learun/adms/form/delete", { schemeInfoId: formSchemeId, keyValue: keyValue }, function (data) {
                                            if (data) {// 删除数据成功
                                                learun.nav.closeCurrent();
            ...
            $header.find('.lr-form-header-submit').on('tap', function () {
                ...
                formreq.push(point);
                learun.httppost(config.webapi + "learun/adms/form/save", formreq, function (data) {
                    if (data) {// 表单数据保存成功
                        if (keyValue) {
            ...
            if (keyValue) {// 详情
                ...
                learun.httpget(config.webapi + "learun/adms/form/data", [{ schemeInfoId: formSchemeId, keyValue: keyValue }], function (data) {
                    if (data) {
                        setTimeout(function () {

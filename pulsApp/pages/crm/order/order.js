(function () {
    ...
    var page = {
        ...
        loadData: function (param, callback, $page) {// 列表加载后台数据
            ...
            learun.httpget(config.webapi + "learun/adms/crm/order/pagelist", _postParam, function (data) {
        ...
        btnClick: function (item, $item, $page) {// 左滑按钮点击事件
            learun.layer.confirm('确定要删除该笔数据吗？', function (_index) {
                if (_index === '1') {
                    ...
                    learun.httppost(config.webapi + "learun/adms/crm/order/delete", item.F_OrderId, function (data) {
                        if (data) {// 删除数据成功
                            page.grid.reload();
                        }

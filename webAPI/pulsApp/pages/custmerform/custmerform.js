 ...
 * 创建人：华羿-前端开发组
 * 日 期：2018.7.12
 * 描 述：华羿移动端框架(ADMS) 自定义表单发布功能-列表页
 */
(function () {
    var page = {
        formSchemeId: '',
        ...
        loadData: function (param, callback, $page) {// 列表加载后台数据
            ...
            learun.httpget(config.webapi + "learun/adms/custmer/pagelist", _postParam, function (data) {
        ...
        btnClick: function (item, $item, $page) {// 左滑按钮点击事件
            learun.layer.confirm('确定要删除该笔数据吗？', function (_index) {
                if (_index === '1') {
                    ...
                    learun.httppost(config.webapi + "learun/adms/form/delete", { schemeInfoId: page.formSchemeId, keyValue: item[page.mainCompontId.toLowerCase()] }, function (data){
                        if (data) {// 删除数据成功
                            page.gird.reload();
                        }

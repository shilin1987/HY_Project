(function () {
    ...
    var page = {
        ...
        init: function ($page, param) {
            ...
            $header.find('.lr-form-header-submit').on('tap', function () {
                ...
                $.each(formDataes, function (id, formData) {
                    if (!fieldMap[id]) {
                        learun.layer.warning('未设置流程表单关联字段,请在流程设计中配置！', function () { }, '华羿提示', '关闭');
                        flag = false;
                        return false;
                    }
        ...
        create: function (info, auditers) {// 提交创建流程
            ...
            learun.httppost(config.webapi + "learun/adms/workflow/create", flowreq, function(data) {
    ...
    function bootstraper($page) {
        ...
        learun.httpget(config.webapi + "learun/adms/workflow/bootstraper", req, function(flowdata){
            learun.layer.loading(false);
            if (flowdata) {

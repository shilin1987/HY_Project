...
namespace Learun.Application.WebApi
{
    ...
    public class BaseApi : NancyModule
    {
        ...
        /// <returns></returns>
        protected  Response Success(Object data, string title, OperationType type, string keyValue, string content)
        {
        ...
        private Response BeforeRequest(NancyContext ctx)
        {
            ...
            token = req.token;
            if (path == "/learun/adms/user/login" || path == "/" || path == "/bgimg" || path == "/learun/adms/user/img" || path == "/learun/adms/desktop/img" || path == "/learun/adms/annexes/down" || path == "/learun/adms/user/plogin" || path == "/learun/adms/user/registered")
            {// 登录接口，默认页面接口不做权限验证处理
                return null;
            }

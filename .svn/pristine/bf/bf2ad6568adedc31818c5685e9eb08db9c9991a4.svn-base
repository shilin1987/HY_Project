//------------------------------------------------------------------------------
// <auto-generated>
//     此代码已从模板生成。
//
//     手动更改此文件可能导致应用程序出现意外的行为。
//     如果重新生成代码，将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

namespace IdentityAuthentication.Database
{
    using System;
    using System.Collections.Generic;
    
    public partial class Hy_Recruit_Blacklist
    {
        public Nullable<int> F_EnabledMark { get; set; }
        public string F_CreateUserId { get; set; }
        public string F_ModifyUserName { get; set; }
        public string F_ModifyUserId { get; set; }
        public Nullable<int> F_DeleteMark { get; set; }
        public Nullable<int> F_SortCode { get; set; }
        public Nullable<System.DateTime> F_ModifyDate { get; set; }
        public string F_CreateUserName { get; set; }
        public Nullable<System.DateTime> F_CreateDate { get; set; }
        public Nullable<int> F_Description { get; set; }
        public string F_IDCard { get; set; }
        public string F_UserName { get; set; }
        public string F_ID { get; set; }
    
        public virtual Hy_Recruit_Blacklist Hy_Recruit_Blacklist1 { get; set; }
        public virtual Hy_Recruit_Blacklist Hy_Recruit_Blacklist2 { get; set; }
    }
}

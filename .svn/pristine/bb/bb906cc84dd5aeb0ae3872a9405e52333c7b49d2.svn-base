...
var bootstrap = function ($, learun) {
    ...
    acceptClick = function (callBack) {
        if (!$('#CSform').lrValidform()) {
            return false;
        }
        var postData = $('#CSform').lrGetFormData();
        var keyValue = $('#F_EnCode').val();
        $.lrSaveForm(top.$.rootUrl + '/Platform/Wl_ChangeShifts/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }

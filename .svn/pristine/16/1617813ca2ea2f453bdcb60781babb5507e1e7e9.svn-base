    ...
    public class ExcelImportService : RepositoryFactory
    {
        ...
        public IEnumerable<ExcelImportFieldEntity> GetModelFieldList(string id, string[] filedArray)
        {
            try
            {
                ...
                return this.BaseRepository().FindList<ExcelImportFieldEntity>(strSql.ToString(), new { importId = id });
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public List<string> GetFiledsList(string[] filedArray)
        {
            try
            {
                ...
                if (entity != null)
                {
                    ...
                    var datas = this.BaseRepository().FindList<DataSourInfo>(strSql.ToString());
                    var lists = datas.Where(e => filedArray.Contains(e.filed)).ToList();

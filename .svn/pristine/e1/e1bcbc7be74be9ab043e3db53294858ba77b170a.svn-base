    ...
    public class Wl_ChangeShiftsService : RepositoryFactory
    {
        ...
        public IEnumerable<UserChangeShiftsEntityDTO> GetUserChangeShiftsList(string fid)
        {
            try
            {
                ...
                var departmentModel = this.BaseRepository().FindList<DepartmentEntity>("select * from lr_base_department");
                var postModel = this.BaseRepository().FindList<PostEntity>("select * from lr_base_post");
                var baseuserModel = this.BaseRepository().FindList<UserEntity>("select * from lr_base_user");
                //var OaReturnInfo= this.BaseRepository().FindList<Hy_Wl_OaReturnInfo_itemEntity>("select * from  Hy_Wl_OaReturnInfo_item  where F_No=(select F_NO from Hy_Wl_ChangeShifts where F_ID='"+fid+"') and  F_NoNum<=4");
                var OaReturnInfo = this.BaseRepository().FindList<Hy_Wl_OaReturnInfo_itemEntity>("select * from  Hy_Wl_OaReturnInfo_item i left join lr_base_user u on i.F_TheApprover=u.F_EnCode  where i.F_No=(select F_NO from Hy_Wl_ChangeShifts where F_ID='" + fid + "') and  i.F_NoNum<=4  order by i.F_NoNum");
                var newUserModel = from u in userModel

...
var bootstrap = function ($, learun) {
    ...
    $("#btn_Search03").on('click', function () {
        ...
        if ($("#F_CUST_CODE").val().length == 0) {
            ...
            learun.layerForm({
                title: "客户产品信息源",
                url: top.$.rootUrl + '/B2B_Code/B2B_PLAN_CREATE/GetDataTable?Fcustom=' + $('#F_CUST_CODE').val(),
                width: 1000,
                height: 650,
                maxmin: false,
                callBack: function (data) {
                    ...
                    //**将选择的数据加载到网格中//
                    //var datarow = { B2B_PLAN_INCOMING_MATERIAL_SUB: mesdata };               
                    //for (var id in datarow) {
                    //    if (!!datarow[id].length && datarow[id].length > 0) {
                    //        $('#B2B_PLAN_INCOMING_MATERIAL_SUB').jfGridSet('refreshdata', datarow[id]);
                    //    }                    
                    //}    
    ...
    $("#lr_refresh").on("click", function () {
        $("#F_CUST_CODE").val("");
        $('#B2B_PLAN_INCOMING_MATERIAL_SUB').jfGridSet('reload', '');
        clearhtml();
    });
    ...
    $('#F_WRITE_MONTH').on('change', function () {
        if ($("#F_WRITE_MONTH").text() == "==请选择==") {
            ...
            $(".jfgrid-head-cell span:eq(7)").html(one);

...
namespace Learun.Application.Web.Areas.LR_OAModule.Controllers
{
    ...
    public class ModuleExportController : MvcControllerBase
    {
        ...
        /// 获取当前登录员工部门信息
        /// </summary>
        /// <param name="F_EnCode">F_EnCode</param>
        /// <returns></returns>
        [HttpGet]
        [AjaxOnly]
        public ActionResult GetDepartmentList()
        {
            var data = recruitmentIBLL.GetDepartmentList("");
            return Success(data);
        }
        #region 扩展：导出
        /// <summary>
        ...
        {
            SR_RecruitmentEntity entity = new SR_RecruitmentEntity();

            

            entity.F_Department = Request.Form["F_Department"];
            entity.F_JobName = Request.Form["F_JobName"];
            entity.F_JonNum = Convert.ToInt32(Request.Form["F_JonNum"]);
            entity.F_CurrentJobName = Convert.ToInt32(string.IsNullOrEmpty(Request.Form["F_CurrentJobName"]) ? 0+"": Request.Form["F_CurrentJobName"]);
            entity.F_DeptMentJonNum = Convert.ToInt32(string.IsNullOrEmpty(Request.Form["F_DeptMentJonNum"])? 0+"": Request.Form["F_DeptMentJonNum"]);
            entity.F_CurrentDeptMentJonNum = Convert.ToInt32(string.IsNullOrEmpty(Request.Form["F_CurrentDeptMentJonNum"]) ? 0+"": Request.Form["F_CurrentDeptMentJonNum"]);
            entity.F_ApplyRecruitNum = Convert.ToInt32(string.IsNullOrEmpty(Request.Form["F_ApplyRecruitNum"]) ? 0 + "" : Request.Form["F_ApplyRecruitNum"]);
            entity.F_DegreeOfEmergency = Request.Form["F_DegreeOfEmergency"];
            entity.F_RequirementTypes = Request.Form["F_RequirementTypes"];
            entity.F_DemandReasonStatement = Request.Form["F_DemandReasonStatement"];
            entity.F_JobRank = Request.Form["F_JobRank"];
            entity.F_JobType = Request.Form["F_JobType"];
            entity.F_RecommendedSalaryRange = Request.Form["F_RecommendedSalaryRange"];
            entity.F_RecruitmentChannels = Request.Form["F_RecruitmentChannels"];
            entity.F_ExpectedArrivalDate = Request.Form["F_ExpectedArrivalDate"];
            entity.F_TheSystemTime = Request.Form["F_TheSystemTime"];
            entity.F_JobDescriptions = Request.Form["F_JobDescriptions"];
            entity.F_MainDuties = Request.Form["F_MainDuties"];
            entity.F_BasicConditions = Request.Form["F_BasicConditions"];
            entity.F_SupposedToKnow = Request.Form["F_SupposedToKnow"];
            entity.F_EssentialSkill = Request.Form["F_EssentialSkill"];
            entity.F_CapacityRequirements = Request.Form["F_CapacityRequirements"];
            entity.F_ExperienceRequirements = Request.Form["F_ExperienceRequirements"];
            entity.F_SpecialRequirements = Request.Form["F_SpecialRequirements"];
            entity.F_DepartmentOfOpinion = Request.Form["F_DepartmentOfOpinion"];
            entity.F_DepartmentOfOpinionOne = Request.Form["F_DepartmentOfOpinionOne"];
            entity.F_DepartmentOfOpinionT = Request.Form["F_DepartmentOfOpinionT"];
            entity.F_HumanResources = Request.Form["F_HumanResources"];
            entity.F_HumanResourcesOne = Request.Form["F_HumanResourcesOne"];
            entity.F_TheSignature_01 = Request.Form["F_TheSignature_01"];
            entity.F_TheSignature_02 = Request.Form["F_TheSignature_02"];
            entity.F_TheSignature_03 = Request.Form["F_TheSignature_03"];
            entity.F_TheSignature_04 = Request.Form["F_TheSignature_04"];
            entity.F_TheSignature_05 = Request.Form["F_TheSignature_05"];
            entity.F_TheSignature_06 = Request.Form["F_TheSignature_06"];
            ...
            if (Request.Files.Count > 0)
            {
                ...
                entity.F_Attachment01Name = f.FileName;
                byte[] buffer = new byte[f.ContentLength];
                f.InputStream.Read(buffer, 0, f.ContentLength);
                base64Str = Convert.ToBase64String(buffer);
                entity.F_Attachment01 = base64Str;
                }
                ...
                HttpPostedFileBase f1 = Request.Files[1];
                entity.F_Attachment02Name = f1.FileName;
                byte[] buffer1 = new byte[f1.ContentLength];
                f1.InputStream.Read(buffer1, 0, f.ContentLength);
                base64Str = Convert.ToBase64String(buffer1);
                entity.F_Attachment02 = base64Str;
                }
                ...
                HttpPostedFileBase f2 = Request.Files[2];
                entity.F_Attachment03Name = f2.FileName;
                byte[] buffer2 = new byte[f2.ContentLength];
                f2.InputStream.Read(buffer2, 0, f2.ContentLength);
                base64Str = Convert.ToBase64String(buffer2);
                entity.F_Attachment03 = base64Str;
                }
            }
            ReturnCommon rc = new ReturnCommon();
            //  HttpFileCollection files = System.Web.HttpContext.Current.Request.Files;
            rc = recruitmentIBLL.SaveEntity(entity);
            if (rc.Status)
            {
                return Success("保存成功！"+ rc.Message);
            }
            else
            {
                return Fail("保存失败！" + rc.Message);
            }
        }
        /// <summary>
        /// 文件转base64
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult UploadFile(string keyValue, SR_RecruitmentEntity entity)
        {
            HttpFileCollection files = System.Web.HttpContext.Current.Request.Files;
            //没有文件上传，直接返回
            ...
            entity.F_Attachment01 = FileEextension;
            entity.F_Attachment02 = "";
            entity.F_Attachment03 = "";

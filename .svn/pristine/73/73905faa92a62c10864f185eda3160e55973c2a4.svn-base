...
namespace Learun.Util
{
    ...
    {
        IWorkbook GetWorkBook(string type)
        {
        ...
        HSSFWorkbook GetHSSFWorkbook(string type)
        {
            HSSFWorkbook wb2003 = null;
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public string  ExportFileToExcel(ExcelConfig excelConfig, string filePath, DataTable dt,string url)
        {
            string filename = "";
            string fullpath = "";
            try
            {
                string fileName = excelConfig.FileName +"";
                ...
                string[] columnTypes = new string[columnCnt];
                ...
                for (int i = 0; i < columnCnt; i++)
                {
                    ...
                    columnTypes[i] = columns[i].DataType.ToString();
                }
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    ...
                    for (int j = 0; j < columnCnt; j++)
                    {
                        ...
                        SetCellValue(cell, dt.Rows[i][j].ToString(), columnTypes[j]);
                    }
                }
        ...
        void SetCellValue(ICell cell, string value, string columnType)
        {
            switch (columnType)
            {
        ...
        public static IWorkbook ExportFileToExcel(DataTable dts, ExcelConfig excelConfig)
        {
            try
            {

                DataTable dt = dts.DefaultView.ToTable(dts.TableName, true,  
                    "F_HrIndicatesOrderNumber", "F_ResumeName","F_WhetherThrough", "F_ProcessTheOrder", "F_OaFlowNumber", "F_Approver", "F_CheckpointName", "F_ApprovalDate", "F_ApprovalResults", "F_ApprovalComments");
                // fileName += fileName + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
                ExcelHelper ex = new ExcelHelper();
                IWorkbook wb = ex.GetWorkBook("xlsx");
                ISheet sheet = wb.CreateSheet();
                ...
                {
                    ICell cell = headerRow.CreateCell(i);
                    foreach (var itemCloName in excelConfig.ColumnEntity)
                    {
                        if (itemCloName.Column.Equals(columns[i].ColumnName))
                        {
                            cell.SetCellValue(itemCloName.ExcelColumn);
                        }
                    }
                    columnTypes[i] = columns[i].DataType.ToString();
                }
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    ...
                    {
                        ICell cell = row.CreateCell(j);
                        ex.SetCellValue(cell, dt.Rows[i][j].ToString(), columnTypes[j]);
                    }
                }
               return wb;
            }

...
var bootstrap = function ($, learun) {
    ...
    var mergeFileChunks = function (file) {
        var fileds = $.trim($("#filed div").html());
        var param = {};
        ...
        param['templateId'] = id;
        param['fileds'] = fileds;
        learun.httpAsyncPost(top.$.rootUrl + "/LR_SystemModule/ExcelImport/ExecuteUserImportExcel", param, function (res) {
    ...
    var page = {
        ...
        init: function () {
            ...
            $('#addfiled').on('click', function () {
                //获取显示字段
                //var txtarea = $.trim($("#importfiled").val());
                //if ($("#filed div").html() == "==请选择==") {
                //    return learun.alert.warning('请选择对应要导入数据库字段');
                //} else {
                //    //给显示字段赋值
                //    if (txtarea == "" || txtarea == null || txtarea == "null") {
                //        txtarea += "" + $("#filed div").html();
                //    } else {
                //        txtarea += "," + $("#filed div").html();
                //    }
                //}
                //$("#importfiled").val(txtarea);
            });
            $('#filed').lrselect({
                url: top.$.rootUrl +'/LR_OrganizationModule/User/GetFiledList',
                text: 'filed',
                method: 'GET',
                value: 'name',
                allowSearch: true,
            ...
            //通过创建字典来改变获取的项目分类
            //$('#filed').lrDataSourceSelect({
            //    code: 'importuser', value: 'name', text: 'filed', type: 'multiple', select: function (val) {
            //        if (val != null && val != "null") {
            //            if (val.filed.indexOf("工号") > -1) {
            //                return learun.alert.warning('工号默认加入,请重新选择');
            //            } else {
            //                var txtarea = $.trim($("#importfiled").val())
            //                if (txtarea == "" || txtarea == null || txtarea == "null") {
            //                    txtarea += "" + val.filed;
            ...
            $('#lr_down_file_btn').on('click', function () {
                var fileds = $("#filed div").html();
                if (fileds.indexOf("工号") < 0) {
                    return learun.alert.warning('工号作为修改条件,必须选择');
                }
                learun.download({ url: top.$.rootUrl + '/LR_SystemModule/ExcelImport/DownTemplateExcelFile', param: { id: id, fileds: fileds, __RequestVerificationToken: $.lrToken }, method: 'POST' });
            });
            ...
            $('#lr_add_file_btn').on('click', function () {
                var fileds = $.trim($("#filed div").html());
                if (fileds == "==请选择==") {
                    return learun.alert.warning('请选择对应要导入数据库字段');
                }

...
namespace Learun.Application.TwoDevelopment.B2B_Code
{
    ...
    /// </summary>
    public class B2B_CUST_PLAN_CONTRAST_WARNService : RepositoryFactory
    {
        #region 获取数据
        ...
        public IEnumerable<B2B_CUST_PLAN_CONTRAST_WARNEntity> GetPageList(Pagination pagination, string queryJson)
        {
            try
            {
                ...
                t.F_TIME
                ");
                strSql.Append("  FROM B2B_CUST_PLAN_CONTRAST_WARN t ");
                ...
                return this.BaseRepository("B2BDatabase").FindList<B2B_CUST_PLAN_CONTRAST_WARNEntity>(strSql.ToString(),dp, pagination);
            }
            catch (Exception ex)
            {
        ...
        public B2B_CUST_PLAN_CONTRAST_WARNEntity GetB2B_CUST_PLAN_CONTRAST_WARNEntity(string keyValue)
        {
            try
            {
                return this.BaseRepository("B2BDatabase").FindEntity<B2B_CUST_PLAN_CONTRAST_WARNEntity>(keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void DeleteEntity(string keyValue)
        {
            try
            {
                this.BaseRepository("B2BDatabase").Delete<B2B_CUST_PLAN_CONTRAST_WARNEntity>(t=>t.F_GUID == keyValue);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
        ...
        public void SaveEntity(string keyValue, B2B_CUST_PLAN_CONTRAST_WARNEntity entity)
        {
            try
            {
                ...
                {
                    entity.Create();
                    entity.F_CREATE_DATE = DateTime.Now;
                    entity.F_CREATE_PERSON = "";
                    entity.F_FLG = 0;

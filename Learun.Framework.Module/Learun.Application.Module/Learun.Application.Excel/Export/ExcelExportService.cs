...
namespace Learun.Application.Excel
{
    ...
    public class ExcelExportService : RepositoryFactory
    {
        ...
        /// <returns></returns>
        public DataTable GetGeneralExport(string modeleId, string queryJson)
        {
            try
            {
                ...
                //业务逻辑
                /**
                 * 1.获取根据id获取数据源导出配置数据源
                 * 2.根据数据源返回对应数据
                 */

                ExcelExportEntity entity = this.BaseRepository().FindEntity<ExcelExportEntity>(e => e.F_ModuleId == modeleId);
                string className = entity.F_InterfaceName;

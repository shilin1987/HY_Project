    ...
    public class OperatorHelper
    {
        ...
        public string AddLoginUser(string account, string appId, string loginMark, bool cookie = true)
        {
            ...
            try
            {
                ...
                redisCache.Write<Dictionary<string, string>>(cacheKeyToken + account, tokenMarkList, CacheId.loginInfo);
                redisCache.Write<Operator>(cacheKeyOperator + operatorInfo.loginMark, operatorInfo, CacheId.loginInfo);
                redisCache.Write<string>(appId, operatorInfo.account, CacheId.loginInfo);
                    ...
                    else
                    {
                        ...
                        if (userInfo == null || userInfo.loadTime == null || userInfo.loadTime <= DateTime.Now.AddMinutes(-5))
                        {
                            ...
                            if (userEntity != null)
                            {
                                ...
                                //userInfo.department = userEntity.F_SecondaryOrganization;
                                userInfo.openId = userEntity.F_OpenId;
                                userInfo.isSystem = userEntity.F_SecurityLevel == 1 ? true : false;

                                //二维码员工信息补全功能添加

                                //userInfo.department = userEntity.department;
                                //userInfo.minDepartment = userEntity.minDepartment;
                                //userInfo.OnJobStatus = userEntity.OnJobStatus;
                                //userInfo.userState = userEntity.userState;
                                //userInfo.oneCardNumber = userEntity.F_OneCardNumber;
        ...
        public void WriteOperateLog(OperateLogModel operateLogModel)
        {
            try
            {
                var opuser = operateLogModel.userInfo;
                if (opuser == null)
                {
                    opuser = LoginUserInfo.Get();
                }

                var logEntity = new LogEntity()
                {
                    F_CategoryId = operateLogModel.CategoryId == 0 ? 2 : operateLogModel.CategoryId,//分类Id 1-登陆2-访问3-操作4-异常
                    F_OperateTypeId = ((int)operateLogModel.type).ToString(),
                    F_OperateType = EnumAttribute.GetDescription(operateLogModel.type),
                    F_OperateAccount = opuser==null?"未找到登录账号": opuser.account,
                    F_OperateUserId = opuser == null ? "未找到登录用户ID" : opuser.userId,
                    F_Module = operateLogModel.title,
                    F_ExecuteResult = 1,
                    F_ExecuteResultJson = "访问地址：" + operateLogModel.url,
                    F_SourceObjectId = operateLogModel.sourceObjectId,
                    F_SourceContentJson = operateLogModel.sourceContentJson,
        ...
        /// 写操作日志
        /// </summary>
        public ReturnCommon WriteOperateLogs(OperateLogModel operateLogModel)
        {
            ...
            try
            {
                ...
                logEntity.F_CategoryId = operateLogModel.CategoryId;
        ...
        public void WriteOperateLogInterfater(OperateLogModel operateLogModel, string userid)
        {
            try
            {
                ...
                if (operateLogModel.userInfo == null)
                {
                    ue = user.GetEntityByAccount(userid);
                }
                LogEntity logEntity = new LogEntity();
                logEntity.F_CategoryId = operateLogModel.CategoryId;
                logEntity.F_OperateTypeId = ((int)operateLogModel.type).ToString();
                logEntity.F_OperateType = EnumAttribute.GetDescription(operateLogModel.type);
                logEntity.F_OperateAccount = ue.F_Account;
                logEntity.F_OperateUserId = ue.F_UserId;
                logEntity.F_Module = operateLogModel.title;
                logEntity.F_ExecuteResult = 1;
                logEntity.F_ExecuteResultJson = "访问地址：" + operateLogModel.url;
                logEntity.F_SourceObjectId = operateLogModel.sourceObjectId;

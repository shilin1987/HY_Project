...
namespace Learun.Application.Organization
{
    ...
    public class DepartmentBLL : DepartmentIBLL
    {
        ...
        /// <returns></returns>
        public List<DepartmentEntity> GetList(string companyId,int isRefreshCache=0)
        {
            try
            {
                if (isRefreshCache == 1)
                {
                    //清空缓存
                    cache.Remove(cacheKey + companyId, CacheId.department);
                }
        ...
        /// <returns></returns>
        public List<DepartmentEntity> GetList(string companyId, string keyWord,int isRefresh)
        {
            try
            {
                List<DepartmentEntity> list = GetList(companyId,isRefresh);
                if (!string.IsNullOrEmpty(keyWord))
                {
                    list = list.FindAll(t => t.F_FullName.Contains(keyWord) || t.F_EnCode.Contains(keyWord) || (t.F_ShortName != null && t.F_ShortName.Contains(keyWord)));
                }
                return list;
            }
            catch (Exception ex)
        ...
        /// <returns></returns>
        public ReturnCommon SaveEntity(string keyValue, DepartmentEntity departmentEntity)
        {
            ReturnCommon msg = new ReturnCommon();
            try
            {
                //如果部门被设置成无效，需判断该部门下的岗位和用户是否都删除了。
                if (departmentEntity.F_EnabledMark == 0)
                {
                    //获取该部门下未删除的岗位，如果存在，返回错误提示
                    List<PostEntity> postList = (List<PostEntity>)new PostService().GetPostListByDepId(keyValue);
                    if (postList.Count > 0)
                    {
                        msg.Status = false;
                        msg.Message = "暂不能设置“部门无效”,请先删除该部门下的岗位和用户！";
                        return msg;
                    }

                    //获取该部门下未删除的用户，如果存在，返回错误提示
                    List<UserEntity> userList = (List<UserEntity>)new UserService().GetUserListByDepId(keyValue);
                    if (postList.Count > 0)
                    {
                        msg.Status = false;
                        msg.Message = "暂不能设置“部门无效”,请先删除该部门下的岗位和用户！";
        ...
        public void UpdateState(string keyValue, int state)
        {
            try
            {
                DepartmentEntity DepartmentEntity = GetEntity(keyValue);
                cache.Remove(cacheKey + DepartmentEntity.F_CompanyId, CacheId.department);
                cache.Remove(cacheKey + "dic", CacheId.department);
                departmentService.UpdateState(keyValue, state);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)

...
namespace Learun.Application.Organization
{
    ...
    public class PostBLL : PostIBLL
    {
        ...
        /// <param name="isRefreshCache">刷新缓存(0,否；1是；)</param>
        /// <returns></returns>
        public List<PostEntity> GetList(string companyId, int isRefreshCache)
        {
            try
            {
                if (isRefreshCache == 1)
                {
                    //清空缓存
                    cache.Remove(cacheKey + companyId, CacheId.post);
                }
        ...
        /// </summary>
        /// <returns></returns>
        public ReturnCommon<List<APPDropDownListModel>> GetPostList()
        {
            var msg = new ReturnCommon<List<APPDropDownListModel>>();
            try
            {
                var list = cache.Read<List<APPDropDownListModel>>(cacheKey + "dropdownlist", CacheId.post);
                if (list == null || list.Count == 0)
                {
                    list = (List<APPDropDownListModel>)postService.GetPostList();
                    cache.Write<List<APPDropDownListModel>>(cacheKey + "dropdownlist", list, TimeSpan.FromMinutes(20), CacheId.post);
                }
                msg.Status = true;
                msg.Data = list;
        ...
        /// <returns></returns>
        public List<PostEntity> GetList(string companyId, string keyword, string departmentId, int isRefresh)
        {
            try
            {
                isRefresh = 1;
                List<PostEntity> list = GetList(companyId, isRefresh);
        ...
        public List<TreeModel> GetTree(string companyId)
        {
            try
            {
                ...
                List<PostEntity> list = GetList(companyId, 0);

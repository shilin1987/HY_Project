...
namespace Learun.Application.Web.Areas.LR_OrganizationModule.Controllers
{
    ...
    public class UserController : MvcControllerBase
    {
        ...
        [HttpGet]
        [AjaxOnly]
        public ActionResult GetPageList(string pagination, string keyword,string companyId,string departmentId,string queryJson)
        {
            Pagination paginationobj = null;
            if (string.IsNullOrEmpty(pagination))
            {
                var data = userIBLL.GetModelPageList(companyId, departmentId, paginationobj, keyword, queryJson);
            ...
            else
            {
                ...
                var data = userIBLL.GetPageList(companyId, departmentId, paginationobj, keyword, queryJson);
                var jsonData = new
                {
                    rows = data,
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult GetFiledList()
        {
            var data = userIBLL.GetFiledList();
            return Success(data);
        }
        /// <summary>
        /// 获取用户列表
        ...
        public ActionResult SaveForm(string keyValue, UserEntity entity)
        {
            ...
            return Success("保存成功！","用户管理", OperationType.Submit, keyValue,JsonConvert.SerializeObject(entity),3);
        }
        /// <summary>
        /// 删除表单数据
        /// </summary>
        ...
        public ActionResult DeleteForm(string keyValue)
        {
            ...
            return Success("删除成功！", "用户管理", OperationType.Submit, keyValue,"虚拟删除用户"+ keyValue, 3);
        }
        /// <summary>
        /// 启用禁用账号
        /// </summary>
        ...
        public ActionResult UpdateState(string keyValue, int state)
        {
            ...
            return Success("操作成功！", "用户管理", OperationType.Submit, keyValue, "启用禁用账号状态为:" + state, 3);
        }
        /// <summary>
        /// 重置用户账号密码
        /// </summary>
        ...
        public ActionResult ResetPassword(string keyValue)
        {
            ...
            return Success("操作成功！", "用户管理", OperationType.Submit, keyValue, "重置用户账号密码", 3);
        }
        #endregion
